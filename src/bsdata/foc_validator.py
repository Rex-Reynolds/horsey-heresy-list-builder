...
class FOCValidator:
    ...
    def __init__(self, detachment_type: str = "Crusade Primary Detachment"):
        ...
        self.detachment_data = loader.get_detachment_by_name(detachment_type)

        if not self.detachment_data:
            logger.warning(f"Detachment '{detachment_type}' not found, using default constraints")
            # Fallback to default primary detachment constraints
        ...
        logger.debug(f"Constraints: {self.constraints}")
    ...
    def _count_categories(self, roster_entries: List[Any]) -> Dict[str, int]:
        ...
        Args:
            roster_entries: List of RosterEntry instances
    ...
    def get_remaining_slots(self, roster_entries: List[Any]) -> Dict[str, Dict[str, int]]:
        ...
        Returns:
            Dict mapping category names to {current, min, max, remaining}
        """
        category_counts = self._count_categories(roster_entries)
        remaining = {}
    ...
    def suggest_additions(self, roster_entries: List[Any]) -> List[str]:
        ...
        Returns:
            List of category names that need more units
        """
