...
export default function UnitBrowser() {
  ...
  return (
    <div className="mx-auto max-w-3xl xl:max-w-5xl">
      ...
      {/* Favorites section */}
      {favorites.size > 0 && !search && !category && (
        <div className="mt-3">
          <div className="category-group-header mb-1.5">
            <svg className="h-3 w-3 shrink-0 text-gold-400" viewBox="0 0 24 24" fill="currentColor">
              <path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
            <span className="font-label text-[11px] font-bold tracking-[0.12em] text-gold-400 uppercase">
              Favorites
            </span>
          ...
          <div className={viewMode === 'list' ? 'space-y-1' : 'grid grid-cols-1 gap-2 xl:grid-cols-2'}>
            {allUnits
              ...
              .map((unit) => {
                const availability = hasDetachments ? getAvailability(unit.unit_type, unit.name, unit.id, unit.constraints) : undefined;
                return renderUnitCard({ unit, availability });
              })}
          </div>
