          ...
          {!expanded && (
            <span
              ...
              className={`quick-add-btn flex h-7 w-7 shrink-0 items-center justify-center rounded-sm border transition-all ${
                isFavorite
                  ? 'border-gold-500/40 bg-gold-600/15 text-gold-400'
                  : 'border-edge-600/20 bg-plate-700/5 text-text-dim/40 hover:border-gold-500/20 hover:text-gold-400/60'
              }`}
              title={isFavorite ? 'Remove from favorites' : 'Add to favorites'}
              aria-label={isFavorite ? `Remove ${unit.name} from favorites` : `Add ${unit.name} to favorites`}
            >
              <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={2} aria-hidden="true">
